...
STYLE_EXTRACTION_PROMPT = """分析以下聊天记录中发言人 {speaker} 的说话风格。

从以下维度描述:
- personality: 性格特征（如: 内向、幽默、毒舌、温柔）
- catchphrases: 口头禅和常用词
- sentence_style: 句式习惯（如: 短句为主、爱用问句、不爱用句号）
- emoji_style: 表情包使用习惯（如: 重度用户、偶尔使用、不用）
- tone: 语气（如: 温柔、直接、调侃）
...
async def extract_style(
    ...
    chat_text = "\n".join(m.get("content", "") for m in sample_msgs)

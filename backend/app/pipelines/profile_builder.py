...
STYLE_EXTRACTION_PROMPT = """你是一位专业的心理语言学分析师。请根据以下聊天记录，深入分析发言人 {speaker} 的性格特质和说话风格。

要求：
- 不要只贴标签，要结合聊天内容给出有洞察力的分析
- 每个 personality 标签都要能从聊天记录中找到具体依据
- 避免"无法判断"，即使消息少也要尽力挖掘

分析维度：
- personality: 性格特征，3-5个词，要求具体有区分度
- catchphrases: 真实的口头禅和常用表达，直接从聊天中摘录
- sentence_style: 句式习惯的深度分析
- emoji_style: 表情包使用习惯（如: 用表情包代替语言、缓和气氛）
- tone: 语气风格（如: 撒娇、强势、理性、阴阳怪气）
